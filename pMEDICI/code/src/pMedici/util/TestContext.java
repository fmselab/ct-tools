...
public class TestContext {
	...
	/**
	 * If the test context is closed (i.e., it has cardinality equals to 1)
	 */
	...
	// if true, the test context is closed when the cardinality is 1
	public static boolean CloseTestContexts = false;

	/**
	 * Builds a new TestContext
	 * 
	 * @param startMdd:       the identifier of the starting MDD (containing the
	 *                        constraints)
	 ...
	 */
	public TestContext(int startMdd, int nParams, boolean useConstraints, MDDManager manager, CitModel model) {
	 ...
	 * @throws InterruptedException
	 */
	private boolean isCompatible(Vector<Pair<Integer, Integer>> tuple, boolean skipFirstStep)
			throws InterruptedException {
		// We must use a test context in a mutex mode
		if (!IN_TEST && !TestContext.LockTCOnlyOnWriting)
			assert (this.testMutex.lockedByCaller() || nCovered == 0);
		...
		// Now, if constraints are available check with the MDD in order to verify if it
	...
	/**
	 * Checks whether the tuple given as parameter is compatible or not by the
	 * current test context without using the MDD
	 * 
	 * @param tuple: the tuple to be checked
	 * @return true if the tuple is compatible, false otherwise
	 */
	...
	/**
	 * Verify with the use of the MDD if the tuple is compatible with this test
	 * context
	 * 
	 * @param tuple: the tuple to be checked
	 * @return true if the tuple is compatible, false otherwise
	...
	public boolean verifyWithMDD(Vector<Pair<Integer, Integer>> tuple) throws InterruptedException {
		...
		// If the intersection is empty, then the tuple is not compatible, otherwise it
		// is
	 ...
	 * TestContext). It does not require the use of an MDD
	 * 
	 * @param tuple: the tuple to be checked
	...
	private void updateMdd(Vector<Pair<Integer, Integer>> tuple, boolean increaseCovered) throws InterruptedException {
		...
		int cardinality = searcher.countPaths();
		assert (cardinality > 0);

		// If the cardinality is equal to 1, it means that only one test can be derived
		// from this test context
		if (cardinality == 1 && CloseTestContexts) {
			close();
		}
	 ...
	 * 
	 * @return the number of paths of the MDD. The higher is the number of paths,
	 *         the higher is the cardinality
	 */
	...
	/**
	 * Returns the status of the current TestContext
	 * 
	 * @return the status of the TC
	 */
