...
class CTWedgeParsingTest {
	...
	def void EnumWithBoolean() {
		// only one parameter with TRUE and FALSE --> it becomes Boolean - TODO to be completed
		var result = parseHelper.parse('''Model Phone Parameters:  display : {TRUE, FALSE}''')
		Assert.assertNotNull(result)
		var errors = result.eResource.errors
		Assert.assertTrue(errors.isEmpty)
		//Assert.assertTrue(typeof(result.parameters.get(0)) == Bool)
		//println(result.parameters.get(0))
		//Assert.assertEquals(2, (result.parameters.get(0) as Bool).elements.size);
	}
