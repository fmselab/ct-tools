...
public class SpecialIssueIWCT2023Test {
	...
	public void testTSCP() throws Exception {
		...
		for (File f : listOfFiles) {
			...
			for (int i = 0; i < N_REP; i++) {
				...
				TestSuite ts1 = null;
				try {
					// ts1 = getACTSTestSuite(Utility.loadModelFromPath(f.getAbsolutePath()),
					// STRENGTH, null);
				...
				if (ts1.getGeneratorTime() != -1) {
					...
					for (int percentage : PERCENTAGE_REMOVAL) {
						...
						List<ctwedge.util.Test> tempTsActs = selectRandomSeeds(
								Utility.loadModelFromPath(f.getAbsolutePath()), ts1, percentage);
						if (tempTsActs.size() > 0)
							tsTemp = new TestSuite(toCSVcode(tempTsActs),
									Utility.loadModelFromPath(f.getAbsolutePath()), ",");
						else
							tsTemp = null;

						// Try with PICT
						tsTempPICT = getPICTTestSuite(Utility.loadModelFromPath(f.getAbsolutePath()), STRENGTH, tsTemp);
						tsTempPICT = mergeTs(tsTempPICT, ts1);
						...
						tsTempACTS = mergeTs(tsTempACTS, ts1);
	...
	/**
	 * Merges the two test suites, as ts1 represents the seeds that we require to be
	 * available in the final test suite
	 * 
	 * @param tsTemp the ts generated by the tool
	 * @param ts1    the seeds
	 * @return a new test suite including seeds
	 */
	private TestSuite mergeTs(TestSuite tsTemp, TestSuite ts1) {
		return mergeTs(tsTemp, ts1.getTests());
	}
	 ...
	 * @return a new test suite including seeds
	 */
	private TestSuite mergeTs(TestSuite tsTemp, List<ctwedge.util.Test> ts1) {
		List<ctwedge.util.Test> testList = tsTemp.getTests();
		for (ctwedge.util.Test t : ts1) {
			Boolean present = false;
			for (ctwedge.util.Test tNew : tsTemp.getTests()) {
				if (tNew.equals(t)) {
					present = true;
					break;
				}
	...
	/**
	 * Given a test suite, a model and a percentage of parameter values to be
	 * removed from each test case, it randomly removes values
	 * 
	 * @param model      the CIT Model
